...
from s3prl.upstream.interfaces import EnsembleFeaturizer
...
class Runner():
    ...
    def __init__(self, args, config):
        ...
        self.upstreams = self._get_upstreams()  # returns a list of upstreams
        self.featurizer = self._get_featurizer()
        self.downstream = self._get_downstream()
        self.all_entries = [*(self.upstreams), self.featurizer, self.downstream]
        self.model_order = ["wav2vec2","hubert","wavlm"]
    ...
    def _get_upstream(self, upstream):
        if "from_hf_hub" in self.args and self.args.from_hf_hub == True:
            from huggingface_hub import snapshot_download

            print(f'[Runner] - Downloading upstream model {upstream} from the Hugging Face Hub')
            filepath = snapshot_download(upstream, self.args.upstream_revision, use_auth_token=True)
    ...
    def _get_upstreams(self):
        ...
        upstreams = []
        model_order_temp = []
        for upstream in self.args.upstream:
            model_order_temp.append(upstream)
            upstreams.append(self._get_upstream(upstream))
        self.model_order = model_order_temp
        return upstreams



    def _get_featurizer(self):
        model = EnsembleFeaturizer(
            upstreams = [upstream.model for upstream in self.upstreams],
            feature_selection = self.args.upstream_feature_selection,
            layer_selection = self.args.upstream_layer_selection,
            layers_for_ws_wav2vec2 = self.args.upstream_layers_for_ws_wav2vec2,
            layers_for_ws_hubert = self.args.upstream_layers_for_ws_hubert,
            layers_for_ws_wavlm = self.args.upstream_layers_for_ws_wavlm,
                ...
                try:
                    ...
                    for upstream in self.upstreams:
                        if upstream.trainable:
                            features.append(upstream.model(wavs))
                        else:
                            with torch.no_grad():
                                features.append(upstream.model(wavs))
