...
class Featurizer(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.layers_for_ws = layers_for_ws
    ...
    def _select_feature(self, features):
        ...
        if isinstance(feature, (list, tuple)) and isinstance(self.layers_for_ws, list):
            feature_temp_list = []
            for i in range(len(self.layers_for_ws)):
                feature_temp_list.append(feature[int(self.layers_for_ws[i])])
            feature = tuple(feature_temp_list)
        return feature
...
class EnsembleFeaturizer(nn.Module):
    def __init__(
        self,
        upstreams: List[UpstreamBase],
        feature_selection: str = "hidden_states",
        upstream_device: str = "cuda",
        layer_selection: int = None,
        layers_for_ws_wav2vec2: list = [] , 
        layers_for_ws_hubert: list = [],
        layers_for_ws_wavlm: list = [] ,       
        # layers_for_ws: list = [] ,
        normalize: bool = False,
        model_order: list = [],
        **kwargs,
        ...
        paired_wavs = [torch.randn(SAMPLE_RATE).to(upstream_device)]
        with torch.no_grad():
            paired_features_list = []
            for upstream in upstreams:
                paired_features_list.append(upstream(paired_wavs))

        for paired_features in paired_features_list:
            if feature_selection not in paired_features:
                if "hidden_states" in paired_features:
                    show(
                        f"[{self.name}] - Warning: {feature_selection} is not a supported args.upstream_feature_selection."
        ...
        self.layers_for_ws_wav2vec2 = layers_for_ws_wav2vec2
        self.layers_for_ws_hubert = layers_for_ws_hubert
        self.layers_for_ws_wavlm = layers_for_ws_wavlm
        self.concat = True

        feature_list = []
        for paired_features,model in zip(paired_features_list,self.model_order):
            feature_list.append(self._select_feature(paired_features,model))
        temp = []
        self.weights = []
        self.weights_model = nn.Parameter(torch.zeros(len(self.model_order)))
        self.layer_num = []
        for i, feature in enumerate(feature_list):
            if isinstance(feature, (list, tuple)):
                self.layer_num.append(len(feature))
                show(
                    f"[{self.name}] - Take a list of {self.layer_num[i]} features and weighted sum them.",
                    file=sys.stderr
                )
                self.weights.append(nn.Parameter(torch.zeros(self.layer_num[i])))
                feature = self._weighted_sum(i, [f.cpu() for f in feature])
            else:
                feature = feature.cpu()
            temp.append(feature)
        ...
        if(self.concat):
            self.output_dim = sum([feature.size(-1) for feature in temp])
        else:
            self.output_dim = feature.size(-1)
        ...
        for i,upstream in enumerate(upstreams):
            if hasattr(upstream, "get_downsample_rates"):
                self.downsample_rates.append(upstream.get_downsample_rates(feature_selection))
                show(
                    f"[{self.name}] - The selected feature {feature_selection}'s downsample rate is {self.downsample_rates[i]}",
                    file=sys.stderr
                )
            else:
                self.downsample_rates.append(round(max(len(wav) for wav in paired_wavs) / feature.size(1)))
                show(
                    f"[{self.name}] - Warning: The provided upstream does not give statis downsample rate"
                    " by the \"get_downsample_rates\" interface (see upstream/example/expert.py)."
                    " The downsample rate is calculated dynamically basing on the shape of the"
                    f" input waveforms v.s. the output features: {self.downsample_rates[i]}",
                    file=sys.stderr
                )

        # assert that the downsample rates of all upstream models in the ensemble must be equal
        assert (all([self.downsample_rates[i]==self.downsample_rates[i-1] for i in range(1,len(self.downsample_rates))])), ("The downsample rate of all upstream models in the ensemble must be equal")
        self.downsample_rate = self.downsample_rates[0]

    def _select_feature(self, features,model):
        feature = features.get(self.feature_selection)
        ...
        layers_for_ws = []
        if (model=="hubert"):
            layers_for_ws = self.layers_for_ws_hubert
        elif (model == "wav2vec2"):
            layers_for_ws = self.layers_for_ws_wav2vec2
        elif (model == "wavlm"):
            layers_for_ws = self.layers_for_ws_wavlm


        if isinstance(feature, (list, tuple)) and isinstance(layers_for_ws, list):
            feature_temp_list = []
            for i in range(len(layers_for_ws)):
                feature_temp_list.append(feature[int(layers_for_ws[i])])
            feature = tuple(feature_temp_list)

        return feature

    def _weighted_sum(self, i, feature):
        assert self.layer_num[i] == len(feature), (
            "If you run into this error, there is a great chance"
            " you are finetuning the upstream with wav2vec2's transformer blocks"
            " in weighted-sum mode (default), including wav2vec2, hubert, and decoar2."
            " These models use the layerdrop technique which causes the different number"
        ...
        _, *origin_shape = stacked_feature.shape
        stacked_feature = stacked_feature.view(self.layer_num[i], -1)
        norm_weights = F.softmax(self.weights[i], dim=-1)
        weighted_feature = (norm_weights.unsqueeze(-1) * stacked_feature).sum(dim=0)
        weighted_feature = weighted_feature.view(*origin_shape)
        return weighted_feature

    def _weighted_sum_model(self,feature):
        stacked_feature = torch.stack(feature, dim=0)
        ...
        _, *origin_shape = stacked_feature.shape
        stacked_feature = stacked_feature.view(len(self.model_order), -1)
        norm_weights = F.softmax(self.weights_model, dim=-1)
        print(norm_weights)
        weighted_feature = (norm_weights.unsqueeze(-1) * stacked_feature).sum(dim=0)
        weighted_feature = weighted_feature.view(*origin_shape)
        return weighted_feature
    ...
    def forward(
        self,
        paired_wavs: List[Tensor],
        paired_features_list: List[Dict[str, Union[Tensor, List[Tensor], Dict[str, Tensor]]]],
    ):
        feature_list = []
        for i, paired_features in enumerate(paired_features_list):
            feature = self._select_feature(paired_features,self.model_order[i])

            if isinstance(feature, (list, tuple)):
                feature = self._weighted_sum(i, feature)
            feature_list.append(feature)

        if (self.concat):
            feature = torch.cat(feature_list, dim=2)
        else:
            feature = self._weighted_sum_model(feature_list)
